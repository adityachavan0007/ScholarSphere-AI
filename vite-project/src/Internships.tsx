            ...
            <div className="max-w-7xl mx-auto relative z-10 flex flex-col h-full mt-6">
                ...
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center text-center gap-4 mb-12">
                    ...
                    <h1 className="text-4xl sm:text-6xl font-extrabold text-white font-outfit tracking-tight">
                        Global <span className="text-indigo-400">Internships</span>
                    </h1>
                ...
                {isLoading ? (
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-20 text-center max-w-3xl mx-auto w-full">
                        ...
                        {searchQuery && (
                            <button onClick={() => setSearchQuery("")} className="mt-6 px-6 py-2.5 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-xl text-sm font-bold font-sans hover:bg-indigo-500/20 transition-colors">
                                Reset Filters
                            </button>
                        )}
                    </motion.div>
                        ...
                        <AnimatePresence>
                            {displayedInternships.map((internship) => (
                                <motion.div
                                    ...
                                    className="bg-zinc-950/60 backdrop-blur-md border border-white/10 rounded-2xl p-6 flex flex-col hover:border-indigo-500/30 transition-colors"
                                >
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="pr-4">
                                            ...
                                            <div className="flex items-center gap-1.5 mt-1">
                                                ...
                                                <p className="text-sm text-indigo-400 font-sans font-medium line-clamp-1">{internship.company}</p>
                                            </div>
                                        </div>
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-6">
                                        {internship.tags.slice(0, 4).map((tag: string) => (
                                            <span key={tag} className="px-2.5 py-1 text-[11px] font-sans font-medium text-slate-400 bg-white/5 border border-white/5 rounded flex items-center gap-1">
                ...
                {hasMoreData && (
                    <div className="flex justify-center mt-4 mb-12">
                        <button
                            ...
                            className="px-6 py-3 bg-zinc-900 border border-indigo-500/30 text-indigo-400 font-sans font-bold text-sm rounded-xl hover:bg-indigo-500/10 transition-all flex items-center gap-2 group"
                        >
