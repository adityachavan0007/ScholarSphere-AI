    ...
    return (
        <div className="w-full max-w-3xl mx-auto mt-24 mb-16 relative z-10">
            <div className="text-center mb-8">
                <h2 className="text-3xl font-bold text-white sm:text-4xl font-sans mb-4">Test the Copilot</h2>
                <p className="text-zinc-400">Enter your core skills and see what the AI finds in milliseconds.</p>
            </div>
            ...
            <div className="w-full overflow-hidden glass-panel border border-white/10 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)]">
                ...
                <div className="flex items-center gap-2 px-4 py-3 bg-black/40 backdrop-blur-md border-b border-white/10">
                    ...
                    <div className="flex items-center gap-2 ml-4 text-xs font-sans text-zinc-400">
                ...
                <div className="p-6">
                    <form onSubmit={handleSimulate} className="flex flex-col gap-4">
                        <div className="flex items-center gap-3">
                            <span className="text-zinc-200 font-sans font-bold">{">"}</span>
                            <input
                                type="text"
                                value={input}
                                onChange={(e) => setInput(e.target.value)}
                                placeholder="e.g., Java, Arduino, IoT, Video Editing..."
                                className="flex-1 bg-transparent border-none outline-none text-white font-sans placeholder:text-slate-600 focus:ring-0"
                            />
                            <button
                                type="submit"
                                className="px-4 py-2 text-xs font-bold text-white transition-colors rounded-lg bg-sky-600 hover:bg-sky-500 font-sans"
                            >
                                EXECUTE
                            </button>
                        </div>
                    ...
                    {isProcessing && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-6 font-sans text-sm text-yellow-400 flex items-center gap-2">
                    ...
                    {showResults && (
                        <motion.div
                            ...
                            className="mt-6 space-y-3 font-sans text-sm"
                        >
                            ...
                            <div className="p-3 border rounded-lg border-sky-500/30 bg-sky-500/5 flex items-start gap-3">
                                <Code className="w-5 h-5 text-zinc-200 mt-0.5" />
                                <div>
                                    <div className="font-bold text-sky-300">Global Tech Hackathon 2026</div>
                                    <div className="text-zinc-400 text-xs mt-1">Requires: {input || "Your Skills"} | Match Score: 98%</div>
                                </div>
                            </div>

                            <div className="p-3 border rounded-lg border-purple-500/30 bg-purple-500/5 flex items-start gap-3">
                                <ArrowRight className="w-5 h-5 text-zinc-200 mt-0.5" />
                                <div>
                                    <div className="font-bold text-purple-300">Open Source Contributor Grant</div>
                                    <div className="text-zinc-400 text-xs mt-1">Stipend: ₹25,000 | Match Score: 92%</div>
                                </div>
                            </div>
                        </motion.div>
                    )}
