import {motion} from 'framer-motion'; export default function T() { return (<>            <motion.div
...
const startMarker = '<motion.divrn                variants={staggerContainer}rn                initial="hidden"rn                animate="show"';
const endMarker = '{/* --- SYSTEM CONFIGURATION POPUP OVERLAY --- */}';

const startIndex = content.indexOf('<motion.divrn                variants={staggerContainer}');
if (startIndex === -1) {
    // try different line endings
    const startIdx2 = content.indexOf('<motion.divn                variants={staggerContainer}');
    if (startIdx2 !== -1) {
...
const regex = /<motion.div[sS]*?variants={staggerContainer}[sS]*?animate="show"[sS]*?className="max-w-5xl mx-auto space-y-8 relative z-10"[sS]*?</motion.div>[sS]*?(?={/* --- SYSTEM CONFIGURATION POPUP OVERLAY --- */})/g;

if (regex.test(content)) {
    let replaced = content.replace(regex, targetJSX + 'nn            ');
    ...
    const sIdx = content.indexOf('            <motion.divn                variants={staggerContainer}');
    const sIdx2 = content.indexOf('            <motion.divrn                variants={staggerContainer}');
    ...
    if (realStart !== -1 && eIdx !== -1) {
        ...
        fs.writeFileSync(filePath, before + targetJSX + 'nn' + after, 'utf8');
